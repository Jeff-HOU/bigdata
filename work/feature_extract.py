...
'''
## replace original feature to the following:
# 1 0. how many kinds of x.delta
'''
utdata_x_diff=np.diff(utdata_x[:,:,0])
test=np.zeros((3000,1))
last_t_index=count_record_num(training_or_testing="t")
for i in range (0,3000):
	test[i]=np.unique(utdata_x_diff[i,0:last_t_index[i]]).size
tfeature[:,0]=test[:,0]

usdata_x_diff=np.diff(usdata_x[:,:,0])
test=np.zeros((100000,1))
last_t_index=count_record_num(training_or_testing="s")
for i in range (0,100000):
	test[i]=np.unique(usdata_x_diff[i,0:last_t_index[i]]).size
sfeature[:,0]=test[:,0]
'''
